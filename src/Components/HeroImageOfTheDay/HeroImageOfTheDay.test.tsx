...
import {HeroImageOfTheDay} from "./HeroImageOfTheDay";
import * as apodApiFetcher from "../ApiFetch/ApodApiFetch";
import React from "react";

describe("Hero Image", () => {
    it('shows video player when media type is video', async () => {
        const body = {
            date: "2020-03-03",
        ...
        jest.spyOn(apodApiFetcher, "getImageOfTheDay").mockImplementation(() => Promise.resolve(body));

        const component = render(<HeroImageOfTheDay/>);
    ...
    it ('shows image when media type is image', async () => {
        ...
        await wait(() => expect(component.getByTestId('image-viewer')).toBeInTheDocument());
        expect(component.queryByTestId('video-player')).toBeNull();
    })
});