...
import {getImageOfTheDay} from "../ApiFetch/ApodApiFetch";
import React from "react";
import {act} from "react-dom/test-utils";

test('check function dynamically switches viewer type depending on content type', async () => {
    let imageDiv:any = '';
    ...
    act(()=>{
        ...
        //@ts-ignore
        jest.spyOn(ApodApiFetch, 'getImageOfTheDay').mockImplementation(() => fakeResponse);

       
        render(<HeroImageOfTheDay/>).getByTestId('imageOfTheDay');
    });
    expect(imageDiv).toContainHTML('img');
});